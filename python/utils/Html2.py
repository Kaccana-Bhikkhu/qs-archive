...
from typing import NamedTuple, TypeVar, Union, Type, Literal
import pyratemp
...
def HiddenBlock(htmlBlock:str,prompt:str = "details",hideWidth:int = 0,blockID:str = "",blockTag:str = "div") -> str:
    """Return hmtl code for a hidden text block.
    htmlBlock: the html to hide.
    prompt: the prompt to display in place of the hidden block.
    hideWidth: if specified, hide this block only on screens smaller than this index.
    blockID: the id of the hidden text block; must be unique on the page.
    blockTag: the tag to enclose the hidden block within."""

    return TruncatedList([htmlBlock],alwaysShow=0,truncateAfter=0,morePrompt=prompt,hideWidth=hideWidth,blockID=blockID,blockTag=blockTag)
...
@contextmanager
def SecondColumnPhoto(a: Airium,photoPath: str,orientation: Literal["portrait","landscape"] = "portrait",imgClass:str = "") -> Iterator[None]:
    """Add a photograph to the right of the html code generated by the with block.
    a: the in-progress Airium object
    photoPath: path to the photo relative to pages/images
    orientation: orientation of the photograph
    imgClass: additional class(es) to apply to the img element"""

    if os.path.isfile(Utils.PosixJoin("pages/images",photoPath)):
        with a.div(Class=f"{orientation}-photo-split"):
            with a.div():
                yield None
            imgClass = " ".join((orientation,imgClass)) if imgClass else orientation
            a.img(Class=imgClass,src=Utils.PosixJoin("../images",photoPath))
    else:
        yield None

def EncloseSecondColumnPhoto(html: str,photoPath: str,orientation: Literal["portrait","landscape"] = "portrait",imgClass:str = "") -> str:
    """Wrap SecondColumnPhoto around some html text."""

    a = Airium()
    with SecondColumnPhoto(a,photoPath,orientation,imgClass):
        a(html)
    return str(a)
