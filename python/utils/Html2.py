...
@contextmanager
def SecondColumnPhoto(a: Airium,photoPath: str,orientation: Literal["portrait","landscape"] = "portrait"):
    ...
    if os.path.isfile(Utils.PosixJoin("pages/images",photoPath)):
        with a.div(Class=f"{orientation}-photo-split"):
            with a.div():
                yield None
            a.img(Class=orientation,src=Utils.PosixJoin("../images",photoPath))
    else:
        yield None
